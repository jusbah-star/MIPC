...
export function AcademicMaterialUploader({
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!selectedTarget || status.kind === 'saving') return;
  ...
  return (
    <form onSubmit={submit} className="mipc-panel grid grid-cols-1 gap-5 p-6 lg:grid-cols-2" aria-labelledby="academic-material-uploader-title">
      ...
      <div className="lg:col-span-2">
        ...
        <select id="academic-material-target" className="mipc-input" value={targetKey} onChange={(event) => setTargetKey(event.target.value)} required disabled={options.length === 0}>
          {options.length === 0 && <option value="">No lessons available for upload</option>}
          {options.map((target) => <option key={target.key} value={target.key}>{target.courseLabel} · {target.scopeLabel}</option>)}
        </select>
      </div>

      <div><label className="mipc-label" htmlFor="academic-material-category">Material category</label><select id="academic-material-category" className="mipc-input" value={category} onChange={(event) => setCategory(event.target.value)}>{categories.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></div>
      <div><label className="mipc-label" htmlFor="academic-material-title">Title</label><input id="academic-material-title" className="mipc-input" value={materialTitle} onChange={(event) => setMaterialTitle(event.target.value)} minLength={3} maxLength={180} required placeholder="e.g. Week 4 reinforced concrete notes" /></div>

      <div className="lg:col-span-2">
        <label className="mipc-label" htmlFor="academic-material-file">Upload file</label>
        <input id="academic-material-file" className="mipc-input file:mr-4 file:rounded-lg file:border-0 file:bg-mipc-navy-50 file:px-3 file:py-2 file:text-sm file:font-semibold file:text-mipc-navy-900" type="file" accept=".pdf,.doc,.docx,.ppt,.pptx,.xls,.xlsx,.txt,.jpg,.jpeg,.png" onChange={(event) => chooseFile(event.target.files?.[0] ?? null)} />
      ...
      <div><label className="mipc-label" htmlFor="academic-material-resource">Optional HTTPS resource link</label><input id="academic-material-resource" className="mipc-input" type="url" pattern="https://.*" value={resourceUrl} onChange={(event) => setResourceUrl(event.target.value)} placeholder="https://…" /></div>
      <div><label className="mipc-label" htmlFor="academic-material-description">Short description</label><input id="academic-material-description" className="mipc-input" value={materialDescription} onChange={(event) => setMaterialDescription(event.target.value)} maxLength={3000} placeholder="What students should use this for" /></div>
      <div className="lg:col-span-2"><label className="mipc-label" htmlFor="academic-material-instructions">Instructions / lesson note</label><textarea id="academic-material-instructions" className="mipc-input" rows={4} value={instructions} onChange={(event) => setInstructions(event.target.value)} maxLength={20000} placeholder="Reading instructions, questions, task details, references, or other guidance…" /></div>

      <div className="flex flex-wrap items-center justify-between gap-4 lg:col-span-2">
        <label className="flex items-center gap-2 text-sm font-semibold text-ink-800"><input type="checkbox" checked={published} onChange={(event) => setPublished(event.target.checked)} className="h-4 w-4 accent-mipc-green-700" /> Publish to students now</label>
        <button type="submit" className="mipc-button-primary" disabled={options.length === 0 || status.kind === 'saving'}>{status.kind === 'saving' ? 'Saving…' : 'Upload material'}</button>
      </div>
