  ...
  return (
    <>
      <section className="relative isolate overflow-hidden bg-mipc-navy-950 text-white">
        <img src="https://mipc.ac.rw/wp-content/uploads/elementor/thumbs/5Q2A8774-scaled-qfzekqx3vew9e5jr1c641sciixqzvx5jos3rkxugf4.jpg" alt="MIPC campus in Musanze" className="absolute inset-0 h-full w-full object-cover" />
        <div className="absolute inset-0 bg-[linear-gradient(90deg,rgba(6,16,34,.98)_0%,rgba(6,16,34,.92)_38%,rgba(21,58,42,.72)_66%,rgba(6,16,34,.18)_100%)]" />
        <div className="relative mx-auto grid min-h-[700px] max-w-7xl items-center gap-10 px-5 py-16 sm:px-8 lg:grid-cols-[1.05fr_.95fr] lg:py-24">
          <div className="max-w-3xl">
            ...
            <p className="mt-7 max-w-2xl text-lg leading-8 text-white/82">MIPC brings technology, hospitality and construction education into the real world — with practical training, close support and a campus community built around integrity, service and excellence.</p>
      ...
      <section className="bg-[#f7f8f5]"><div className="mx-auto grid max-w-7xl gap-10 px-5 py-20 sm:px-8 lg:grid-cols-[.9fr_1.1fr] lg:py-24"><div><p className="mipc-eyebrow !text-mipc-green-700">Why MIPC</p><h2 className="mt-3 text-4xl font-bold tracking-tight text-mipc-navy-950">Education that feels useful from day one.</h2><p className="mt-5 max-w-xl leading-7 text-ink-700">MIPC combines technical training with personal development and community values. The aim is simple: help students leave campus with stronger skills, stronger judgement and the confidence to contribute.</p><a href="https://mipc.ac.rw/about" target="_blank" rel="noreferrer" className="mipc-button-secondary mt-7 !border-mipc-navy-900/15">Read the MIPC story</a></div><div className="grid gap-4 sm:grid-cols-3">{[{ icon: ShieldCheckIcon, title: 'Integrity', text: 'Clear standards, responsible decisions and respect for every learner.' }, { icon: BookOpenIcon, title: 'Practice', text: 'Workshops, projects and real-world learning across our programmes.' }, { icon: UsersIcon, title: 'Belonging', text: 'A supportive campus where students can learn, grow and contribute.' }].map(({ icon: Icon, title, text }) => <div key={title} className="rounded-[1.35rem] border border-mipc-navy-900/10 bg-white p-6 shadow-academic"><Icon className="h-7 w-7 text-mipc-green-700" /><h3 className="mt-5 text-xl font-bold text-mipc-navy-950">{title}</h3><p className="mt-2 text-sm leading-6 text-ink-600">{text}</p></div>)}</div></div></section>
