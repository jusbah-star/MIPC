...
function genericSuccess() {
  return NextResponse.json(
    { ok: true, mode: 'link', message: GENERIC_SIGN_IN_MESSAGE },
    { status: 200 }
  );
}
...
export async function POST(request: Request) {
  try {
    ...
    await enforceRateLimit(
      `portal-link-account:${portalIdentityKey(portal, email, registrationNumber)}`,
      5,
      15 * 60 * 1000
    );
    ...
    // Deliberately return the same public response for missing, inactive, and wrong-role accounts.
    // This keeps the endpoint from becoming an account/role enumeration oracle.
    if (!matchesProfile) return genericSuccess();
    ...
    if (error) {
      // Do not expose dispatch failure only for known accounts; that would restore enumeration.
