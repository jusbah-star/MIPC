...
export default async function RegistrarStudents({
  ...
  const [studentsResult, departmentsResult, cohortsResult] = await Promise.all([
    admin.from('profiles').select('id,full_name,email,registration_number,department_id,cohort_id,year_of_study,registration_status,account_status').eq('role','student').order('full_name'),
    admin.from('departments').select('id,name,code').order('name'),
    admin.from('cohorts').select('id,name,department_id').order('name')
  ]);
  if(studentsResult.error||departmentsResult.error||cohortsResult.error) throw new Error('Student register could not be loaded.');
