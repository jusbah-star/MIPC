export const PORTAL_ROLES = Object.freeze(['student', 'lecturer', 'admin']);
...
export function profilePortalDestination(profile) {
  if (!profile || profile.account_status !== 'active' || !isPortalRole(profile.role)) {
...
export function safePortalNext(profile, requestedNext) {
  ...
  if (!profileCanAccessPortal(profile, profile?.role)) return fallback;
